#include "../../EASUI/EASUI.h"




int main()
{

        EASUI__INIT(5);


        EASUI_WINDOW WINDOW_1, WINDOW_2, WINDOW_3, WINDOW_4, WINDOW_5;
        EASUI_LABEL LABEL_1;


        SET_NEW_EASUI_WINDOW(&WINDOW_1, "Window 1", 8, 800, 800, TRUE);
        SET_NEW_EASUI_WINDOW(&WINDOW_2, "Window 2", 8, 600, 600, TRUE);
        SET_NEW_EASUI_WINDOW(&WINDOW_3, "Window 3", 8, 800, 800, TRUE);
        SET_NEW_EASUI_WINDOW(&WINDOW_4, "Window 4", 8, 600, 600, TRUE);
        SET_NEW_EASUI_WINDOW(&WINDOW_5, "Window 5", 8, 800, 800, TRUE);


        //SET_NEW_EASUI_LABEL(&LABEL_1, &WINDOW_2, 0, 0, 30, 30, 9, 1024);


        WINDOW_1.START(&WINDOW_1);
        WINDOW_2.START(&WINDOW_2);
        WINDOW_3.START(&WINDOW_3);
        WINDOW_4.START(&WINDOW_4);
        WINDOW_5.START(&WINDOW_5);


        EASUI__RUN();


        printf("EASUI ENDED PERFECTLY, THAT MEANS IT HAS A WORKING WINDOW MANAGER\n");


        return 0;

}
